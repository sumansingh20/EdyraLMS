...
export default function ExamAttemptPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f2f3f7] flex items-center justify-center">
        <div className="bg-white p-8 rounded-md border border-[#dfe3e8] shadow text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#0f47ad] mx-auto mb-4"></div>
          <p className="text-[#4b5563]">Loading exam session...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#f2f3f7] flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-md border border-[#dfe3e8] shadow text-center max-w-md">
          <div className="text-red-500 text-6xl mb-4">⚠</div>
          <h2 className="text-xl font-bold text-[#1d2125] mb-2">Session Error</h2>
          ...
          <div className="flex gap-3 justify-center">
            <button
              ...
              className="bg-[#2f7d32] text-white px-6 py-2 rounded-md hover:bg-[#1f6b1f]"
            >
              Retry
            </button>
            <button
              onClick={() => router.push('/exam/login')}
              className="bg-[#0f47ad] text-white px-6 py-2 rounded-md hover:bg-[#0a3b8f]"
            >
              Back to Login
            </button>
          </div>
    ...
    return (
      <div className="min-h-screen bg-[#f2f3f7] flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-md border border-[#dfe3e8] shadow text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-[#1d2125] mb-2">Exam Submitted</h2>
          <p className="text-[#4b5563] mb-2">Your exam has been submitted successfully.</p>
  ...
  if (terminated) {
    return (
      <div className="min-h-screen bg-[#f2f3f7] flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-md border border-[#dfe3e8] shadow text-center max-w-md">
          <div className="text-red-500 text-6xl mb-4">✕</div>
          <h2 className="text-xl font-bold text-[#1d2125] mb-2">Session Terminated</h2>
          <p className="text-[#4b5563] mb-4">{terminationReason}</p>
          <p className="text-sm text-[#6b7280] mb-4">
  ...
  return (
    <div className="min-h-screen bg-[#f2f3f7] select-none" style={{ userSelect: 'none' }}>
      {/* Fullscreen reminder */}
      ...
      {(requireCamera || requireMicrophone) && (
        <div className="fixed right-4 top-[78px] z-50 overflow-hidden rounded-lg bg-gray-900 shadow-xl" style={{ width: 180 }}>
          {requireCamera && (
      ...
      {/* Header - Fixed */}
      <header className="fixed left-0 right-0 top-0 z-50 bg-white text-[#1d2125] shadow-[0_6px_24px_rgba(140,152,164,0.13)]">
        <div className="flex min-h-[70px] items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-4">
            {branding.portalLogoUrl ? (
              <div className="h-10 w-16 rounded-md border border-[#dfe3e8] bg-white p-1">
                <img src={branding.portalLogoUrl} alt={branding.portalLogoAlt || 'Portal logo'} className="h-full w-full object-contain" />
              </div>
            ) : (
              <div className="h-10 w-10 rounded-md bg-white text-[#1d2125] flex items-center justify-center font-bold text-xs">LMS</div>
            )}
            <div>
              <p className="text-xs text-[#6b7280] tracking-wide uppercase">{branding.institutionName}</p>
              <h1 className="max-w-xs truncate text-base font-bold">{examTitle}</h1>
            </div>
          </div>

          <div className="flex items-center space-x-6">
            {branding.institutionLogoUrl && (
              <div className="hidden h-9 w-20 rounded-md border border-[#dfe3e8] bg-white p-1 sm:flex">
                <img src={branding.institutionLogoUrl} alt={branding.institutionLogoAlt || 'Institution logo'} className="h-full w-full object-contain" />
              </div>
            )}
            ...
            {(requireCamera || requireMicrophone) && (
              <div className="flex items-center space-x-2 text-xs">
                {requireCamera && (
                  <span className={`rounded px-1.5 py-0.5 ${cameraStream ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                    CAM {cameraStream ? 'ON' : 'OFF'}
                  </span>
                )}
                {requireMicrophone && (
                  <span className={`rounded px-1.5 py-0.5 ${micStream ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
            ...
            {/* Timer */}
            <div className={`flex items-center space-x-2 ${remainingTime <= 300 ? 'text-red-600 animate-pulse' : 'text-[#1d2125]'}`}>
            ...
            {/* Violation counter */}
            <div className={`flex items-center space-x-1 ${violationCount > 0 ? 'text-amber-600' : 'text-[#1d2125]'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="text-sm">
              ...
              <span className="text-[#6b7280]">/{totalQuestions} answered</span>
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Warning banner */}
      {warningMessage && (
        <div className="fixed left-0 right-0 top-[70px] z-50 bg-[#f9cb4f] py-2 text-center text-sm font-semibold text-[#1d2125] animate-pulse">
          {warningMessage}
        </div>
      )}

      {/* Main content */}
      <div className="flex pb-20 pt-[70px]">
        {/* Question navigation sidebar */}
        <aside className="fixed bottom-20 left-0 top-[70px] w-24 overflow-y-auto border-r border-[#dfe3e8] bg-white shadow-sm">
          <div className="p-2">
            <p className="text-xs text-[#6b7280] text-center mb-2">Questions</p>
                ...
                <button
                  ...
                  className={`w-6 h-6 text-xs font-medium rounded ${
                    currentQuestion === index
                      ? 'bg-[#0f47ad] text-white'
                      : getQuestionStatus(q.id) === 'answered'
                      ? 'bg-[#2f7d32] text-white'
                      : 'bg-[#eef4ff] text-[#4b5563] hover:bg-[#d1def5]'
                  }`}
                >
                  {index + 1}
                </button>
        ...
        {/* Question area */}
        <main className="flex-1 ml-24 px-6 py-4">
          {currentQ && (
            <div className="max-w-3xl mx-auto">
              <div className="bg-white rounded-lg border border-slate-200 shadow p-6">
                ...
                <div className="flex items-center justify-between mb-4 pb-4 border-b">
                  ...
                  <span className="text-sm font-semibold text-[#0f47ad]">
                ...
                <div className="space-y-3">
                        ...
                        <label
                          ...
                          className={`flex items-center p-4 border rounded-md cursor-pointer transition-colors ${
                            answers.get(currentQ.id)?.selectedOption === optIndex
                              ? 'border-[#0f47ad] bg-[#eef4ff]'
                              : 'border-[#dfe3e8] hover:border-[#cfd8e3] hover:bg-[#f8f9fb]'
                          }`}
                        >
                          <input
                            type="radio"
                  ...
                  {currentQ.type === 'true_false' && (
                    <div className="flex space-x-4">
                      {['True', 'False'].map((opt, optIndex) => (
                        <label
                          key={opt}
                          className={`flex-1 flex items-center justify-center p-4 border rounded-md cursor-pointer transition-colors ${
                            answers.get(currentQ.id)?.selectedOption === optIndex
                              ? 'border-[#0f47ad] bg-[#eef4ff]'
                              : 'border-[#dfe3e8] hover:border-[#cfd8e3] hover:bg-[#f8f9fb]'
                          }`}
                  ...
                  {(currentQ.type === 'short_answer' || currentQ.type === 'numerical') && (
                    <>
                      ...
                      <input
                        ...
                        className="w-full p-4 border border-[#dfe3e8] rounded-md focus:ring-2 focus:ring-[#d1def5] focus:border-[#0f47ad]"
                  ...
                  {currentQ.type === 'essay' && (
                    <textarea
                      ...
                      onChange={(e) => handleAnswerChange(currentQ.id, { textAnswer: e.target.value })}
                      rows={8}
                      className="w-full p-4 border border-[#dfe3e8] rounded-md focus:ring-2 focus:ring-[#d1def5] focus:border-[#0f47ad] resize-none"
                  ...
                  {currentQ.type === 'code' && (
                    <div>
                      {currentQ.codeLanguage && (
                        <div className="mb-2 text-sm text-[#6b7280] bg-[#eef4ff] px-3 py-1 rounded inline-block">
                        ...
                        return (
                          <div key={pairIndex} className="flex items-center gap-4 mb-3 p-3 bg-gray-50 rounded-lg">
                            ...
                            <select
                              ...
                              className="flex-1 p-2 border border-[#dfe3e8] rounded-md focus:ring-2 focus:ring-[#d1def5]"
                            >
                ...
                {answers.has(currentQ.id) && (
                  <button
                    ...
                    className="mt-4 text-sm text-[#6b7280] hover:text-[#0f47ad]"
                  >
                    Clear answer
                  </button>
                )}
      ...
      {/* Footer - Fixed */}
      <footer className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#dfe3e8] shadow-lg z-50">
        <div className="flex items-center justify-between px-6 py-3">
          <button
            onClick={() => setCurrentQuestion(Math.max(0, currentQuestion - 1))}
            disabled={currentQuestion === 0}
            className="px-6 py-2 bg-[#0f47ad] text-white rounded-md hover:bg-[#0a3b8f] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ← Previous
          </button>
          ...
          {allowManualSubmission && (
            <button
              ...
              className="px-6 py-2 bg-[#2f7d32] text-white rounded-md hover:bg-[#1f6b1f] font-medium"
            >
              Submit Exam
            </button>
          )}
